...
\paragraph{}Prenons par exemple l'instruction "Si l'agent voit une ressource, il la récupère", symbolisée par les classes PerceptFood et ActionPick :
\paragraph{}
AJOUTER LE CODE !
\paragraph{}
Pour cette instruction nous avons donc accès à son percept et son action, la vérification des percepts se fait de la manière suivante :
\begin{itemize}
\item Nous récupérons la liste des percepts nécessaires à l'instruction.
\item Nous récupérons la liste des percepts de l'unité.
\item Nous récupérons la valeur de chaque percepts demandés.
\item Si toutes leurs valeurs valent vrai on exécute l'action demandée.
\end{itemize}

La valeur de chaque percept de l'unité est mise à jour à chaque unité de temps 
...
Notre objectif était de simplifier l'ajout de percepts et d'actions à une unité. Pour ce faire nous avons utiliser les type "delegate" propre à Unity dont la déclaration est semblable à  une signature de méthode. Elle a une valeur de retour et un nombre quelconque de paramètres de type quelconque.
Nous l'avons donc utilisé afin d'encapsuler les méthodes correspondant aux actions et aux percepts.
