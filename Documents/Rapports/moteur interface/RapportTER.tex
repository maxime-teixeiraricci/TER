...
\section{Amélioration possible}
L'interface a été grandement améliorée par rapport à ce qu'elle était lorsque nous l'avons reçu, cependant, il reste beaucoup d'éléments qui aurait pu être implémentés avec plus de temps, ou de bug qui aurait pu être résout. Nous avons listé ces différents points :
\subsection{Fonctionnalité}
\begin{itemize}
\item InfoBulle apparaissant lors du passage du curseur au dessus d'un éléments de l'interface. 
\item Zone de comparaison de deux comportements à la fois, soit deux unités différentes soit la même unité mais dans deux équipes différentes.
...
\item Nouveau type de condition pour remplacer "Vie 75\%", "Vie 50\%", ... qui deviendrait une seule pièce avec un menu déroulant, qui contiendrait la liste des valeurs. 
\item Bruitage du jeu séparé de la musique du jeu. Dans l’intérêt que l'on puisse modifier l'un sans changer l'autre.
\item Une checkBox dans le menu des paramètres pour activer/désactiver le tir allié des unités.
\item Les boutons UNDO et REDO ne concerne que la création et la suppression des pièces de comportement. Une amélioration serait donc que ces boutons puisse aussi annuler le déplacement d'une pièce.
...
\item Changement de l'aspect de l'éditeur suivant le jeu(fond d'écran, code couleur des pièces, musiques, etc...).
...
\item Bouton en jeu pour ouvrir l'éditeur de comportement sans arrêter la partie (et pouvoir modifier les comportements en direct).
\end{itemize}


\newpage
\part{L'avenir du projet}
\chapter{Pistes envisageable.}
\begin{itemize}
